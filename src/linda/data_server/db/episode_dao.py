    ...
    def get_recent_episodes(self, since_time, stash=None, origin=0):
        try:
            if int(origin) == 0:
                ...
                sql = "SELECT e.id, e.show_id, e.url, e.priority,e.title, e.category, e.tag, e.audit,e.description,e.duration, c.site_name, c.site_code,e.send FROM %s AS e \
                        LEFT JOIN %s AS c ON e.site_id = c.site_id \
                        WHERE e.create_time > %%s AND e.send = 0 AND (e.format_id > 1 or e.site_id = 2) " % \
                        (self._episode_tb_name, self._site_tb_name, )
            else:
                sql = "SELECT e.id, e.show_id, e.type, e.url, e.priority,e.title, e.category, e.tag,e.audit,e.send,e.description,e.duration, c.site_name, c.site_code FROM %s AS e \
                        LEFT JOIN %s AS c ON e.site_id = c.site_id \
