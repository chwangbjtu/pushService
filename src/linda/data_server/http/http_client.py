...
class HttpClient(object):
    ...
    def post_data(self, url, body):
        try:
            ...
            #head = {"Content-Type": "application/json","Accept": "*/*","Connection":"keep alive","User-Agent": "Fiddler"}
            #head = {"Content-type":"text/html;charset=utf-8"}
            head = {"Content-Type":"application/json"}
            request = httpclient.HTTPRequest(url=url, method="POST", body=body, headers=head)
    ...
    def get_data(self, url):
        try:
            ...
            log.app_log.error("get error: [%s] [%s]" % (url, e))
            return ('', '')
