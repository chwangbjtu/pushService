...
int http_request::pack_post(char *buf, const int len)
{
	...
	for(; c_iter != c_iter_end; c_iter++)
	{
		...
		//str2escape(c_iter->first.c_str(),c_iter->first.length(), key);
		//str2escape(c_iter->second.c_str(),c_iter->second.length(), val);
        key.append(c_iter->first.c_str(),c_iter->first.length());
        val.append(c_iter->second.c_str(),c_iter->second.length());
    ...
    num = snprintf(buf+pos, len-pos, "%s", _req._content.c_str());    
