...
class HttpClient(object):
    ...
    def get_data(self, url, ua=None, cookies=None, timeout=10, read_body=True, read_all=True):
        if cookies:
            for c in cookies:
    ...
    def post_data(self, url, body, ua=None, cookies=None, timeout=10, read_body=True, read_all=True):
        if cookies:
            for c in cookies:
                self._cookies.set_cookie(make_cookie(c['name'], c['value']))
        req = urllib2.Request(url)
...
if __name__ == "__main__":
    import json
    hc = HttpClient(with_cookie=True)
